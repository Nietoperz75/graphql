...
public class FakeSmartSearchDataResolver {
    ...
    public List<SmartSearchResult> getSmartSearch(@InputArgument(name = "keyword") Optional<String> keyword){
        var smartSearchResult = new ArrayList<SmartSearchResult>();
        if(keyword.isEmpty()){
            smartSearchResult.addAll(FakeHelloDataSource.HELLO_LIST);
            smartSearchResult.addAll(FakeBookDataSource.BOOK_LIST);
