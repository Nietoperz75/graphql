...
public class FakeBookDataResolverTest {
    ...
    void testAllBooks(){
        ...
        List<Author> authors = dgsQueryExecutor.executeAndExtractJsonPathAsObject(graphqlQueryRequest, "data.books[*].author", new TypeRef<List<Author>>(){

        });
        assertNotNull(authors);
        assertEquals(titles.size(), authors.size());

        List<Integer> releaseYears = dgsQueryExecutor.executeAndExtractJsonPathAsObject(graphqlQueryRequest, "data.books[*].released.year", new TypeRef<List<Integer>>(){
    ...
    void testBooksWithInput(){
        ...
        var releaseHistoryInput = ReleaseHistoryInput.newBuilder()
                ...
                .build();
        var graphqlQuery = BooksByReleasedGraphQLQuery.newRequest().releasedInput(releaseHistoryInput)
                .build();
        var projectionRoot = new BooksProjectionRoot().released().year().printedEdition();

        var graphqlQueryRequest = new GraphQLQueryRequest(graphqlQuery, projectionRoot).serialize();

        List<Integer> releaseYears = dgsQueryExecutor.executeAndExtractJsonPath(graphqlQueryRequest, "data.booksByReleased[*].released.year");
        ...
        List<Boolean> releasePrintedEditions = dgsQueryExecutor.executeAndExtractJsonPath(graphqlQueryRequest, "data.booksByReleased[*].released.printedEdition");
        Set<Boolean> uniqueReleasePrintedEdition = new HashSet<>(releasePrintedEditions);
        assertNotNull(uniqueReleasePrintedEdition);
        assertTrue(uniqueReleasePrintedEdition.size()<=1);
        if(!uniqueReleasePrintedEdition.isEmpty()){
            assertTrue(uniqueReleasePrintedEdition.contains(expectedPrintedEdition));
        }
    }
}
